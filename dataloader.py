...
def get_dataloader(root_path, train_file="train.csv", test_file = "test.csv", batch_size=8, device='cuda', save_path=None, reload=None):
    ...
    train_data, val_data = torchtext.data.TabularDataset.splits(path=root_path, train=train_file, validation=test_file, format='csv', fields=data_fields, skip_header=False)
